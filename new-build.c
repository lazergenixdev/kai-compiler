...
typedef enum {
	...
	Identifier_Type_Internal_Type,     // Kai__
	Identifier_Type_Internal_Function, // kai__
} Identifier_Type;
...
void generate_zero_value(String_Builder* builder, Kai_Type type)
{
    ASSERT(type != NULL);
    switch (type->id)
    {
        ...
        break;default: {
            sb_appendf(builder, "[todo zero value %i]", type->id);
        }
    }
}
...
void generate_type(String_Builder* builder, Kai_Type type, Kai_u32 flags)
{
    ...
    switch (type->id)
    {
        ...
        break;case KAI_TYPE_ID_FLOAT: {
            ...
            sb_appendf(builder, "f%u", info->bits);
        }
        break;case KAI_TYPE_ID_POINTER: {
...
void generate_procedure_definition(String_Builder* builder, Kai_Expr* expr)
{
    ...
    if (d->value->id != KAI_EXPR_PROCEDURE) return;

    Kai_string name = d->name;
    Kai_Expr_Procedure* p = (Kai_Expr_Procedure*)d->value;
    ...
    Kai_bool api = KAI_BOOL(d->flags & KAI_FLAG_DECL_EXPORT);
    shput(g_identifier_map, cstr(name),
        api ? Identifier_Type_Function : Identifier_Type_Internal_Function);
...
const char* map_binary_operator(Kai_u32 op, Kai_bool use_alternative)
{
    ...
    case KAI_MULTI('<', '<',,): return "<<";
    case KAI_MULTI('>', '>',,): return ">>";
    case KAI_MULTI('-', '>',,): return "CAST"; // special case
    case '[': return "INDEX"; // special case
...
void generate_expression(String_Builder* builder, Kai_Expr* expr, int prec, Kai_u32 flags)
{
    ...
    ASSERT(expr != NULL);
    if (expr->tag != NULL) // keep?
    {
        if (kai_string_equals(expr->tag->name, KAI_STRING("const")))
            sb_append(builder, "const ");
    }
	switch (expr->id)
	{
	break;case KAI_EXPR_IDENTIFIER: {
        ...
        break;case Identifier_Type_Internal_Type:     prefix = "Kai__";
        break;case Identifier_Type_Internal_Function: prefix = "kai__";
        break;default: {
            if (flags & GE_TYPE)
                prefix = "Kai_";
        }
    ...
    break;case KAI_EXPR_NUMBER: {
        ...
        case KAI_TYPE_ID_INTEGER: {
            ...
            Kai_u64 value64 = kai_number_to_u64(n->value);
            switch (type->bits)
            {
            case 8:
            case 16:
            case 32:
            {
                sb_appendf(builder, "%i", (int)value64);
            } break;
            case 64: {
                sb_appendf(builder, "%lluLL", value64);
            } break;
            }
            if (!type->is_signed)
                sb_append(builder, "u");
        ...
        default: {
            ...
            if (kai_number_is_integer(n->value))
                sb_appendf(builder, "%lluLL", kai_number_to_u64(n->value));
            else
                sb_appendf(builder, "%f", kai_number_to_f64(n->value));
        } break;
        }
    }
    ...
    break;case KAI_EXPR_PROCEDURE_CALL: {
        ...
        generate_expression(builder, p->proc, TOP_PRECEDENCE, flags);
        sb_append(builder, "(");
		for (Kai_Expr* current = p->arg_head; current != NULL; current = current->next)
        {
            generate_expression(builder, current, TOP_PRECEDENCE, flags);
...
void generate_statement(String_Builder* builder, Kai_Stmt* stmt, int depth, int increase_depth_on_non_compound)
{
	...
	case KAI_STMT_DECLARATION: {
        ...
        sb_append(builder, " = ");
        if (decl->value == NULL) {
            generate_zero_value(builder, decl->this_type);
        }
        else {
            generate_expression(builder, decl->value, TOP_PRECEDENCE, 0);
        }
        sb_append(builder, ";\n");
	} break;
	...
	case KAI_STMT_ASSIGNMENT: {
		...
		tab(depth);
		generate_expression(builder, ass->dest, TOP_PRECEDENCE, 0);
		...
		generate_expression(builder, ass->value, TOP_PRECEDENCE, 0);
		sb_append(builder, ";\n");
	} break;
	...
	case KAI_STMT_IF: {
		...
		generate_expression(builder, _if->condition, TOP_PRECEDENCE, 0);
		sb_append(builder, ")\n");
	...
	case KAI_STMT_WHILE: {
		...
		generate_expression(builder, whi->condition, TOP_PRECEDENCE, 0);
		sb_append(builder, ")\n");
...
void generate_typedef(String_Builder* builder, Kai_Expr* expr)
{
    ...
    Kai_Stmt_Declaration* d = (Kai_Stmt_Declaration*)expr;
    if (d->value == NULL) return;
    if (d->value->id == KAI_EXPR_STRUCT)
    {
        const char* name = cstr(d->name);
        sb_appendf(builder, "typedef struct Kai_%s Kai_%s;\n", name, name);
    }
    else if (d->value->id == KAI_EXPR_ENUM)
    {
        Kai_Expr_Enum* e = (Kai_Expr_Enum*)d->value;
...
void generate_procedure_typedef(String_Builder* builder, Kai_Expr* expr)
{
    ...
    Kai_Stmt_Declaration* d = (Kai_Stmt_Declaration*)expr;
    if (d->value == NULL) return;
    if (d->value->id == KAI_EXPR_PROCEDURE_TYPE)
    {
        Kai_Expr_Procedure_Type* p = (Kai_Expr_Procedure_Type*)d->value;
...
void generate_enum(String_Builder* builder, Kai_Expr* expr)
{
    ...
    Kai_Stmt_Declaration* d = (Kai_Stmt_Declaration*)expr;
    if (d->value == NULL) return;
    if (d->value->id != KAI_EXPR_ENUM) return;
    Kai_Expr_Enum* e = (Kai_Expr_Enum*)d->value;
...
void generate_struct(String_Builder* builder, Kai_Expr* expr)
{
    ...
    Kai_Stmt_Declaration* d = (Kai_Stmt_Declaration*)expr;
    if (d->value == NULL) return;
    if (d->value->id != KAI_EXPR_STRUCT) return;
    Kai_Expr_Struct* s = (Kai_Expr_Struct*)d->value;
...
void generate_procedure_implementation(String_Builder* builder, Kai_Expr* expr)
{
    ...
    ASSERT(d->value->id == KAI_EXPR_PROCEDURE);
    Kai_Expr_Procedure* p = (Kai_Expr_Procedure*)d->value;
