 ...
 * define  KAI_USE_CPP_API     to enable C++ API (not complete)
 *
...
#define KAI__MAKE_VERSION_STRING2(X,Y,Z) KAI__MAKE_VERSION_STRING(X,Y,Z) // garbage helper macro because macros suck.
...
#define KAI_VERSION_STRING_V "v" KAI__MAKE_VERSION_STRING2(KAI_VERSION_MAJOR,KAI_VERSION_MINOR,KAI_VERSION_PATCH)
...
typedef struct {
    ...
    Kai_Memory_Allocator  memory;      // input
...
KAI_API(void)
	kai_get_version(Kai_u32* out_Major, Kai_u32* out_Minor, Kai_u32* out_Patch);
...
KAI_API(Kai_Result)
	kai_create_syntax_tree(Kai_Syntax_Tree_Create_Info* Info, Kai_Syntax_Tree* Syntax_Tree);

KAI_API(void)
	kai_destroy_syntax_tree(Kai_Syntax_Tree* Syntax_Tree);
...
KAI_API(void*)
	kai_find_procedure(Kai_Program Program, char const* Name, char const* opt_Type);
...
KAI_API(void)
	kai_create_memory(Kai_Memory_Allocator* Memory);

KAI_API(void)
	kai_destroy_memory(Kai_Memory_Allocator* Memory);

KAI_API(void)
	kai_memory_set_debug(Kai_Memory_Allocator* Memory, Kai_u32 Debug_Level);

KAI_API(Kai_u32)
	kai_memory_usage(Kai_Memory_Allocator* Memory);
...
KAI_API(Kai_Debug_String_Writer*)
	kai_debug_stdout_writer(void);

KAI_API(void)
	kai_debug_open_file_writer(Kai_Debug_String_Writer* out_Writer, const char* Path);

KAI_API(void)
	kai_debug_close_file_writer(Kai_Debug_String_Writer* Writer);
